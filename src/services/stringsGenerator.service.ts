...
export class StringsGeneratorService {
    static async getStringsToChangeTable(sequelize: Sequelize, table_schema: string, table_name: string): Promise<{ upString: string, downString: string }> {
        ...
        for(const column in tableInModel) {
            ...
            else { //column is missing in Db -> add
                up_string += `await queryInterface.addColumn({tableName: '${table_name}', schema: '${table_schema}'}, '${column}', {`;
                console.log("ADDING COLUMN")
                up_string += ModelService.beautifyModelColumn(tableInModel);
                up_string += `},);`
                down_string += `await queryInterface.removeColumn({tableName: '${table_name}', schema: '${table_schema}'}, '${column}', {transaction: t});`
            }
        }
        //column is missing in Model -> delete
        for(const column in tableInDb) {
            if (!Object.keys(tableInModel as {}).includes(column)) {
